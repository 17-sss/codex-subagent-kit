...
이 패키지는 더 이상 단순 bootstrap만은 아니다. stable command surface 대부분이 TypeScript에서 동작하고 있으며, parity hardening과 npm release 작업이 끝날 때까지 Python 버전을 source of truth로 유지한다.

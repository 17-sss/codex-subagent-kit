...
이 패키지는 더 이상 단순 bootstrap만은 아니다. stable command surface 대부분이 TypeScript에서 동작하며, 이제 npm release target이기도 하다. Python 앱은 저장소 안에 legacy implementation, fallback 경로, 그리고 npm-first surface 밖의 experimental 명령 보관소로 남겨둔다.
...
6. `doctor` 포팅
7. `usage` 포팅
8. Python 계약과 비교하는 fixture 기반 parity test 추가
9. npm metadata와 publish workflow 준비
10. TypeScript package 전용 CI와 publish automation 추가
...
이제 TypeScript package가 npm release target이다. Python 앱은 저장소 안에서 legacy implementation, reference 경로, 그리고 npm-first surface에 포함되지 않은 experimental 명령의 보관소로 계속 남겨둔다.

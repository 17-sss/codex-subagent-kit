...
- Last Updated: 2026-03-20T14:12:00+09:00
...
- project-scope install은 `.codex/agents/*.toml`과 함께 `.codex/orchestrator/team.toml` seed 및 control-plane scaffold 디렉터리를 만든다.
- `catalog`와 선택 로직은 project/global `.toml` source를 함께 발견하고 precedence(`project > global > built-in`)를 적용한다.
- `panel` 명령은 이제 `team.toml`을 읽어 minimal terminal topology를 렌더링한다.

## Current Objective

- current `panel` 명령을 runtime-aware control panel 및 `tmux` / `cmux` 흐름으로 확장하고, 루트 orchestrator topology를 실제 런타임 시각화로 이어 준다.
...
- `panel --project-root <path>`는 generated `team.toml`을 텍스트 트리로 렌더링한다.
...
In progress
- minimal panel renderer를 runtime-aware control panel에 연결하는 방식 정리
To confirm
...
- `team.toml` 기반 minimal terminal panel renderer 추가
Validation run
...
- `team.toml` seed를 직접 확인할 수 있는 최소 terminal topology view가 생겼다.
...
- current panel은 static topology 렌더러일 뿐 queue/runtime/pane 상태를 아직 보여주지 않는다.
Workaround
...
- minimal terminal panel renderer
...
- `panel` 명령은 `operator -> orchestrator -> workers` topology를 렌더링함
Not run yet
- runtime-aware terminal control panel 생성 흐름

## Next Actions

1. current `panel` 명령을 queue/runtime/pane 상태가 보이는 runtime-aware control panel로 확장한다.
...
- `reference/legacy_shell_control_plane/`를 보고 queue/runtime/pane 상태를 어떤 식으로 패널에 올릴지 결정한다.

## Resume Prompt

Continue this project from `docs/HANDOFF.md`. First verify the repo still matches the notes, then implement the next unfinished action: expand the current `panel` command into a runtime-aware control panel and connect it to `tmux` / `cmux` launcher flow, using the migrated legacy control-plane assets only as reference, not as the primary runtime.

...
- [x] T001 Extend `src/codex_subagent_kit/control_plane.py` with queue-command selection and dispatch-ticket creation.
- [x] T002 Add CLI surface in `src/codex_subagent_kit/cli.py` for opening a dispatch from the current queue state.
...
- [x] T006 Extend `src/codex_subagent_kit/control_plane.py` with result-apply transitions for `completed`, `failed`, and `cancelled`.
- [x] T007 Update runtime-state mutation so active roles return to the correct post-result status.
- [x] T008 Add CLI surface in `src/codex_subagent_kit/cli.py` for applying a dispatch result.

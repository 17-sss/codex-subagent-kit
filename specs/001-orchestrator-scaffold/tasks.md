...
- [x] T001 Align canonical agent TOML output in `src/codex_orchestrator/generator.py` with the chosen Codex-compatible structure.
...
- [x] T004 Add team manifest and scaffold generation logic in `src/codex_orchestrator/generator.py` for project-scope installs.
- [x] T005 Ensure the seed manifest contains one explicit root orchestrator and the remaining worker agents.
- [x] T006 Report created and preserved scaffold paths in CLI output from `src/codex_orchestrator/cli.py`.
...
- [x] T009 Update `src/codex_orchestrator/tui.py` so project-scope install still works cleanly with scaffold generation and root orchestrator rules.
